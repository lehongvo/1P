...
generate_transferred_files_scenarios() {
    ...
    for i in {1..4}; do
        ...
        transfer_price="TH_PRCH_${DATE_PATTERN}${timestamp}_SOA_TRANSFERRED_${i}.csv"
...
EOF
    ...
    for i in {1..3}; do
        ...
        transfer_promo="TH_PROMPRCH_${DATE_PATTERN}${timestamp}_SOA_TRANSFERRED_${i}.csv"
...
generate_processed_files_scenarios() {
    ...
    for i in {1..3}; do
        ...
        processed_price="TH_PRCH_${DATE_PATTERN}${timestamp}_SOA_PROCESSED_${i}.csv"
...
EOF
    ...
    for i in {1..2}; do
        ...
        processed_promo="TH_PROMPRCH_${DATE_PATTERN}${timestamp}_SOA_PROCESSED_${i}.csv"
...
generate_orphaned_files_scenarios() {
    ...
    for i in {1..2}; do
        ...
        orphan_price="TH_PRCH_${DATE_PATTERN}${timestamp}_SOA_ORPHANED_${i}.csv"
...
EOF
    ...
    orphan_promo="TH_PROMPRCH_${DATE_PATTERN}${timestamp}_SOA_ORPHANED.csv"
...
generate_timestamp_variation_scenarios() {
    ...
    for i in "${!time_patterns[@]}"; do
        ...
        time_test_price="TH_PRCH_${DATE_PATTERN}${timestamp}_SOA_TIME_TEST_$((i+1)).csv"
...
generate_file_metadata_scenarios() {
    ...
    metadata_rich_file="TH_PRCH_${DATE_PATTERN}${timestamp}_SOA_METADATA_RICH.csv"
...
upload_fetch_soa_files_to_docker() {
    ...
    for file in $FETCH_SOA_DIR/TH_PRCH_${DATE_PATTERN}*.csv; do
        if [ -f "$file" ]; then
    ...
    for file in $FETCH_SOA_DIR/TH_PROMPRCH_${DATE_PATTERN}*.csv; do
        if [ -f "$file" ]; then
