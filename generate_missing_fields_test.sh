...
generate_price_missing_fields() {
    ...
    missing_item_id_price="TH_PRCH_${DATE_PATTERN}${timestamp}.csv"
    ...
    second=$((RANDOM % 60))
    timestamp=$(printf "%02d%02d%02d" $hour $minute $second)
    missing_price_field="TH_PRCH_${DATE_PATTERN}${timestamp}.csv"
    ...
    second=$((RANDOM % 60))
    timestamp=$(printf "%02d%02d%02d" $hour $minute $second)
    missing_start_date_price="TH_PRCH_${DATE_PATTERN}${timestamp}.csv"
    ...
    second=$((RANDOM % 60))
    timestamp=$(printf "%02d%02d%02d" $hour $minute $second)
    missing_end_date_price="TH_PRCH_${DATE_PATTERN}${timestamp}.csv"
    ...
    second=$((RANDOM % 60))
    timestamp=$(printf "%02d%02d%02d" $hour $minute $second)
    empty_values_price="TH_PRCH_${DATE_PATTERN}${timestamp}.csv"
...
generate_promotion_missing_fields() {
    ...
    second=$((RANDOM % 60))
    timestamp=$(printf "%02d%02d%02d" $hour $minute $second)
    missing_promo_id="TH_PROMPRCH_${DATE_PATTERN}${timestamp}.csv"
    ...
    second=$((RANDOM % 60))
    timestamp=$(printf "%02d%02d%02d" $hour $minute $second)
    missing_discount="TH_PROMPRCH_${DATE_PATTERN}${timestamp}.csv"
    ...
    second=$((RANDOM % 60))
    timestamp=$(printf "%02d%02d%02d" $hour $minute $second)
    multiple_missing="TH_PROMPRCH_${DATE_PATTERN}${timestamp}.csv"
    ...
    second=$((RANDOM % 60))
    timestamp=$(printf "%02d%02d%02d" $hour $minute $second)
    empty_values_promo="TH_PROMPRCH_${DATE_PATTERN}${timestamp}.csv"
...
generate_valid_files_for_comparison() {
    ...
    second=$((RANDOM % 60))
    timestamp=$(printf "%02d%02d%02d" $hour $minute $second)
    valid_price="TH_PRCH_${DATE_PATTERN}${timestamp}.csv"
    ...
    second=$((RANDOM % 60))
    timestamp=$(printf "%02d%02d%02d" $hour $minute $second)
    valid_promo="TH_PROMPRCH_${DATE_PATTERN}${timestamp}.csv"
...
upload_to_docker_container() {
    ...
    if [ $CLEAN_DOCKER -eq 1 ]; then
        ...
        docker exec $DOCKER_CONTAINER rm -f $SFTP_1P_PRICE/TH_PRCH_*.csv >/dev/null 2>&1 || true
        docker exec $DOCKER_CONTAINER rm -f $SFTP_1P_PROMOTION/TH_PROMPRCH_*.csv >/dev/null 2>&1 || true
        echo -e "${GREEN}✅ Cleaned existing files${NC}"
    fi
    
    # Upload Price Files to 1P
    echo -e "${YELLOW}📤 Uploading Price files with missing fields...${NC}"
    for file in $MISSING_FIELDS_DIR/TH_PRCH_*.csv; do
        if [ -f "$file" ]; then
    ...
    for file in $MISSING_FIELDS_DIR/TH_PROMPRCH_*.csv; do
        if [ -f "$file" ]; then
