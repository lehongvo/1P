...
echo -e "${YELLOW}📤 Extracting start_transfer_loop function...${NC}"
...
for file in "${TEST_FILES[@]}"; do
    if [ -f "$file" ]; then
        ...
        if grep -q "start_transfer_loop" "$file"; then
            echo -e "${BLUE}  ✅ start_transfer_loop already exists in $file${NC}"
            continue
        fi
        
        # Add the function at the end of the file
        echo "$TRANSFER_LOOP_FUNCTION" >> "$file"
        echo -e "${GREEN}  ✅ Added start_transfer_loop to $file${NC}"
    else
        echo -e "${RED}  ❌ File not found: $file${NC}"
    fi
done

echo -e "${GREEN}🎉 Completed adding start_transfer_loop function to all test files!${NC}"
