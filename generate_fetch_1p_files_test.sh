...
generate_successful_fetch_scenarios() {
    ...
    for i in {1..5}; do
        ...
        success_price="TH_PRCH_${DATE_PATTERN}${timestamp}_FETCH_SUCCESS_${i}.csv"
        success_price_path="$FETCH_1P_DIR/$success_price"
        
        cat > "$success_price_path" << EOF
item_id,price,start_date,end_date
SUCCESS_ITEM_${i}_001,$(( RANDOM % 1000 + 100 )).$(( RANDOM % 100 )),$INPUT_DATE,$(date -d "$INPUT_DATE + 30 days" +%Y-%m-%d)
SUCCESS_ITEM_${i}_002,$(( RANDOM % 1000 + 100 )).$(( RANDOM % 100 )),$INPUT_DATE,$(date -d "$INPUT_DATE + 30 days" +%Y-%m-%d)
SUCCESS_ITEM_${i}_003,$(( RANDOM % 1000 + 100 )).$(( RANDOM % 100 )),$INPUT_DATE,$(date -d "$INPUT_DATE + 30 days" +%Y-%m-%d)
EOF
        echo -e "${GREEN}    Generated: $success_price${NC}"
    done
    ...
    for i in {1..3}; do
        ...
        success_promo="TH_PROMPRCH_${DATE_PATTERN}${timestamp}_FETCH_SUCCESS_${i}.csv"
...
SUCCESS_PROMO_${i}_001,$discount,$INPUT_DATE,$end_date
SUCCESS_PROMO_${i}_002,$discount,$INPUT_DATE,$end_date
SUCCESS_PROMO_${i}_003,$discount,$INPUT_DATE,$end_date
EOF
        echo -e "${GREEN}    Generated: $success_promo${NC}"
    done
...
generate_large_directory_scenarios() {
    ...
    for i in {1..50}; do
        ...
        large_test_file="TH_PRCH_${DATE_PATTERN}${timestamp}_LARGE_DIR_TEST_${i}.csv"
...
generate_file_format_variety_scenarios() {
    ...
    # Standard CSV
    timestamp1=$(printf "%02d%02d%02d" $((16 + RANDOM % 2)) $((RANDOM % 60)) $((RANDOM % 60)))
    standard_csv="TH_PRCH_${DATE_PATTERN}${timestamp1}_STANDARD_FORMAT.csv"
...
EOF
    ...
    # CSV with BOM
    timestamp2=$(printf "%02d%02d%02d" $((17 + RANDOM % 2)) $((RANDOM % 60)) $((RANDOM % 60)))
    bom_csv="TH_PRCH_${DATE_PATTERN}${timestamp2}_WITH_BOM.csv"
    ...
    # Large CSV file
    timestamp3=$(printf "%02d%02d%02d" $((18 + RANDOM % 2)) $((RANDOM % 60)) $((RANDOM % 60)))
    large_csv="TH_PRCH_${DATE_PATTERN}${timestamp3}_LARGE_FILE.csv"
...
upload_fetch_1p_files_to_docker() {
    ...
    for file in $FETCH_1P_DIR/TH_PRCH_${DATE_PATTERN}*.csv; do
        if [ -f "$file" ]; then
    ...
    for file in $FETCH_1P_DIR/TH_PROMPRCH_${DATE_PATTERN}*.csv; do
        if [ -f "$file" ]; then
